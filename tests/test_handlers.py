...
def test_plan_transfer_batches_keeps_current_destination_across_non_route_captions() -> None:
    ...
    batches, error_text = plan_transfer_batches(
        [
            [
                _fake_message(
                    chat_id=77,
            ...
            [
                _fake_message(
                    ...
                    video=_fake_video(
                        file_id='f2',
                        file_name=_stored_filename(source_group, source_sub_group, _CLIP_ID_2),
                    ),
                )
            ],
            [
                _fake_message(
                    ...
                    video=_fake_video(
                        file_id='f3',
                        file_name=_stored_filename(source_group, source_sub_group, _CLIP_ID_3),
                    ),
                )
            ],
        ],
    ...
    assert batches[0].destination_group == ClipGroup(universe=Universe.WEST, year=2025, season=Season.S1)
    assert [(clip.source_group, clip.clip_id) for clip in batches[0].clips] == [
        (source_group, _CLIP_ID_1),
        (source_group, _CLIP_ID_2),
        (source_group, _CLIP_ID_3),
    ]
...
async def test_document_only_mp4_dispatch_to_restricted_clip_action_menu() -> None:
    ...
    expected_reply_markup = selection_keyboard(
        buttons=[
            ...
            for action in [IntakeAction.PRODUCE]
        ],
        back_button=InlineKeyboardButton(
            text=IntakeAction.CANCEL.title(),
            callback_data=IntakeActionCallbackData(
...
@pytest.mark.asyncio
async def test_document_mp4_only_batch_dispatches_to_document_menu_without_route() -> None:
    scheduler = _FakeScheduler()
    buffer = ChatMessageBuffer()
    services = _services(clip_store=SimpleNamespace(), scheduler=scheduler, buffer=buffer)
    settings = _settings()
...
async def test_standalone_text_and_video_clip_dispatch_to_route_only_menu() -> None:
    ...
    reply_markup = message.answer.await_args.kwargs['reply_markup']
    _assert_three_rows(reply_markup)
    assert _keyboard_rows(reply_markup) == [['Route'], ['—'], ['Cancel']]


@pytest.mark.asyncio
async def test_standalone_text_and_mp4_document_dispatch_to_route_only_menu() -> None:
    scheduler = _FakeScheduler()
    buffer = ChatMessageBuffer()
    services = _services(clip_store=SimpleNamespace(), scheduler=scheduler, buffer=buffer)
    settings = _settings()
...
@pytest.mark.asyncio
async def test_caption_only_video_clip_dispatches_to_route_only_menu() -> None:
    scheduler = _FakeScheduler()
    buffer = ChatMessageBuffer()
    services = _services(clip_store=SimpleNamespace(), scheduler=scheduler, buffer=buffer)
    settings = _settings()
...
@pytest.mark.asyncio
async def test_caption_only_document_clip_dispatches_to_route_only_menu() -> None:
    scheduler = _FakeScheduler()
    buffer = ChatMessageBuffer()
    services = _services(clip_store=SimpleNamespace(), scheduler=scheduler, buffer=buffer)
    settings = _settings()
...
@pytest.mark.asyncio
async def test_mixed_video_document_and_standalone_text_dispatch_to_route_only_menu() -> None:
    scheduler = _FakeScheduler()
    buffer = ChatMessageBuffer()
    services = _services(clip_store=SimpleNamespace(), scheduler=scheduler, buffer=buffer)
    settings = _settings()
...
@pytest.mark.asyncio
async def test_whitespace_only_text_and_clip_dispatches_to_generic_clip_menu_without_route() -> None:
    scheduler = _FakeScheduler()
    buffer = ChatMessageBuffer()
    services = _services(clip_store=SimpleNamespace(), scheduler=scheduler, buffer=buffer)
    settings = _settings()
...
@pytest.mark.asyncio
async def test_non_route_caption_video_clip_dispatches_to_generic_clip_menu_without_route() -> None:
    scheduler = _FakeScheduler()
    buffer = ChatMessageBuffer()
    services = _services(clip_store=SimpleNamespace(), scheduler=scheduler, buffer=buffer)
    settings = _settings()
...
@pytest.mark.asyncio
async def test_non_route_caption_document_clip_dispatches_to_document_menu_without_route() -> None:
    scheduler = _FakeScheduler()
    buffer = ChatMessageBuffer()
    services = _services(clip_store=SimpleNamespace(), scheduler=scheduler, buffer=buffer)
    settings = _settings()
...
def test_classify_route_request_marks_missing_or_noncanonical_filename_as_external() -> None:
    ...
    assert [clip.kind for clip in buffered_clips] == [
...
@pytest.mark.asyncio
async def test_track_store_final_menu_layout_from_sub_season_path() -> None:
    settings = _settings()
    buffer = ChatMessageBuffer()
    buffer.append(
        _fake_message(chat_id=42, message_id=1, photo=_fake_photo(file_id='photo-1'), caption='Artist\nTitle'),
    ...
    track_store = SimpleNamespace(
        list_tracks=AsyncMock(
            return_value={track_store_module.SubSeason.A: []},
        ),
        store=AsyncMock(),
    )
    services = _services(clip_store=SimpleNamespace(), track_store=track_store, buffer=buffer)
    menu_message = _fake_message(text='Select action:', chat_id=42, message_id=501)
    state = _FakeState()
    bot = SimpleNamespace(get_file=AsyncMock(), download_file=AsyncMock())
    year = date.today().year - 1
    ...
    assert state.current_state == TrackStoreFlow.final.state
    assert _keyboard_rows(menu_message.edit_text.await_args.kwargs['reply_markup']) == [['Skip'], ['Fetch'], ['Back']]


@pytest.mark.asyncio
async def test_track_store_final_menu_layout_from_cover_source_path() -> None:
    settings = _settings()
    album_id = '018f05c1f1a37b348d291f53a1ca4a1'
    track_store = SimpleNamespace(
        list_tracks=AsyncMock(
            return_value={
                track_store_module.SubSeason.A: [
                    track_store_module.TrackInfo(
                        id='018f05c1f1a37b348d291f53a1ca4a2',
                        album_id=album_id,
                        artists=('artist',),
                        title='Album A',
                        has_instrumental=False,
    ...
    buffer = ChatMessageBuffer()
    buffer.append(
        _fake_message(chat_id=42, message_id=1, text='https://www.youtube.com/watch?v=abc123\nArtist\nTitle'),
        chat_id=42,
    )
    services = _services(clip_store=SimpleNamespace(), track_store=track_store, buffer=buffer)
    menu_message = _fake_message(text='Select action:', chat_id=42, message_id=502)
    state = _FakeState()
    bot = SimpleNamespace(get_file=AsyncMock(), download_file=AsyncMock())
    year = date.today().year - 1
...
@pytest.mark.asyncio
async def test_track_store_final_menu_layout_from_album_path() -> None:
    settings = _settings()
    album_id = '018f05c1f1a37b348d291f53a1ca4b1'
    track_store = SimpleNamespace(
        list_tracks=AsyncMock(
            return_value={
                track_store_module.SubSeason.A: [
                    track_store_module.TrackInfo(
                        id='018f05c1f1a37b348d291f53a1ca4b2',
                        album_id=album_id,
                        artists=('artist',),
                        title='Album A',
                        has_instrumental=False,
    ...
    services = _services(clip_store=SimpleNamespace(), track_store=track_store, buffer=buffer)
    menu_message = _fake_message(text='Select action:', chat_id=42, message_id=503)
    state = _FakeState()
    bot = SimpleNamespace(get_file=AsyncMock(), download_file=AsyncMock())
    year = date.today().year - 1
...
@pytest.mark.asyncio
async def test_track_store_link_plus_audio_supports_url_text_first_with_explicit_metadata(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    settings = _settings()
    year = date.today().year - 1
    ...
    download_audio_as_opus.assert_not_awaited()
    stored_track = track_store.store.await_args.kwargs['track']
    assert stored_track.artists == ('Artist 1', 'Artist 2')
    assert stored_track.title == 'Title'
    assert stored_track.cover == FileBytes(data=b'url-cover-1', extension=Extension.JPG)
    assert stored_track.audio == FileBytes(data=b'opus-audio-1', extension=Extension.OPUS)
    menu_message.answer.assert_awaited_once_with(text='Done')
    assert services.chat_message_buffer.peek_raw(42) == []
    assert state.current_state is None
...
@pytest.mark.asyncio
async def test_track_store_link_plus_audio_supports_audio_first_and_metadata_fallback(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    settings = _settings()
    year = date.today().year - 1
    ...
    buffer = ChatMessageBuffer()
    buffer.append(
        _fake_message(chat_id=42, message_id=1, audio=_fake_audio(file_id='audio-2', file_name='one.mp3')),
        chat_id=42,
    )
    buffer.append(
        _fake_message(chat_id=42, message_id=2, text='https://www.youtube.com/watch?v=abc123'),
        chat_id=42,
    )
    services = _services(clip_store=SimpleNamespace(), track_store=track_store, buffer=buffer)
    menu_message = _fake_message(text='Select action:', chat_id=42, message_id=9832)
    state = _FakeState()
    bot = SimpleNamespace(
        get_file=AsyncMock(return_value=SimpleNamespace(file_path='audio-path-2')),
        download_file=AsyncMock(return_value=BytesIO(b'audio-2')),
    )
    monkeypatch.setattr(track_store_execution_module, 'to_opus', AsyncMock(return_value=b'opus-audio-2'))
    fetch_track_info = AsyncMock(
    ...
    download_audio_as_opus.assert_not_awaited()
    stored_track = track_store.store.await_args.kwargs['track']
    assert stored_track.artists == ('Fallback 1', 'Fallback 2')
    assert stored_track.title == 'Fallback Title'
    assert stored_track.cover == FileBytes(data=b'url-cover-2', extension=Extension.JPG)
    assert stored_track.audio == FileBytes(data=b'opus-audio-2', extension=Extension.OPUS)
    menu_message.answer.assert_awaited_once_with(text='Done')
    assert services.chat_message_buffer.peek_raw(42) == []
    assert state.current_state is None
...
@pytest.mark.asyncio
async def test_track_store_link_plus_audio_with_user_metadata_ignores_metadata_fallback_errors(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    settings = _settings()
    year = date.today().year - 1
    ...
    buffer.append(
        _fake_message(chat_id=42, message_id=2, audio=_fake_audio(file_id='audio-6', file_name='one.mp3')),
        chat_id=42,
    )
    services = _services(clip_store=SimpleNamespace(), track_store=track_store, buffer=buffer)
    menu_message = _fake_message(text='Select action:', chat_id=42, message_id=9836)
    state = _FakeState()
    bot = SimpleNamespace(
        get_file=AsyncMock(return_value=SimpleNamespace(file_path='audio-path-6')),
        download_file=AsyncMock(return_value=BytesIO(b'audio-6')),
    )
    monkeypatch.setattr(track_store_execution_module, 'to_opus', AsyncMock(return_value=b'opus-audio-6'))
    ...
    assert stored_track.artists == ('Artist 1', 'Artist 2')
    assert stored_track.title == 'Title'
    assert stored_track.cover == FileBytes(data=b'url-cover-6', extension=Extension.JPG)
    assert stored_track.audio == FileBytes(data=b'opus-audio-6', extension=Extension.OPUS)
    menu_message.answer.assert_awaited_once_with(text='Done')
    assert services.chat_message_buffer.peek_raw(42) == []
    assert state.current_state is None
...
@pytest.mark.asyncio
async def test_track_store_link_plus_audio_with_metadata_failure_invalidates_when_fallback_required(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    settings = _settings()
    year = date.today().year - 1
    ...
    buffer = ChatMessageBuffer()
    buffer.append(
        _fake_message(chat_id=42, message_id=1, audio=_fake_audio(file_id='audio-3', file_name='one.mp3')),
        chat_id=42,
    )
    buffer.append(
        _fake_message(chat_id=42, message_id=2, text='https://www.youtube.com/watch?v=abc123'),
    ...
    services = _services(clip_store=SimpleNamespace(), track_store=track_store, buffer=buffer)
    menu_message = _fake_message(text='Select action:', chat_id=42, message_id=9833)
    state = _FakeState()
    bot = SimpleNamespace(
        get_file=AsyncMock(return_value=SimpleNamespace(file_path='audio-path-3')),
        download_file=AsyncMock(return_value=BytesIO(b'audio-3')),
    )
...
@pytest.mark.asyncio
async def test_track_store_link_plus_audio_with_cover_failure_invalidates_download_failed(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    settings = _settings()
    year = date.today().year - 1
    ...
    buffer.append(
        _fake_message(chat_id=42, message_id=2, audio=_fake_audio(file_id='audio-4', file_name='one.mp3')),
        chat_id=42,
    )
    services = _services(clip_store=SimpleNamespace(), track_store=track_store, buffer=buffer)
    menu_message = _fake_message(text='Select action:', chat_id=42, message_id=9834)
    state = _FakeState()
    bot = SimpleNamespace(
        get_file=AsyncMock(return_value=SimpleNamespace(file_path='audio-path-4')),
...
@pytest.mark.asyncio
async def test_track_store_link_plus_audio_rejects_audio_caption_when_url_text_present() -> None:
    settings = _settings()
    track_store = SimpleNamespace(list_tracks=AsyncMock(), store=AsyncMock())
    buffer = ChatMessageBuffer()
    buffer.append(
        _fake_message(
            chat_id=42,
            message_id=1,
            audio=_fake_audio(file_id='audio-5', file_name='one.mp3'),
            caption='Artist\nTitle',
        ),
        chat_id=42,
    )
    ...
    services = _services(clip_store=SimpleNamespace(), track_store=track_store, buffer=buffer)
    menu_message = _fake_message(text='Select action:', chat_id=42, message_id=9835)
    state = _FakeState()
...
@pytest.mark.asyncio
@pytest.mark.parametrize(
    ('from_step', 'expected_state', 'expected_rows'),
    [
        (TrackStoreStep.SUB_SEASON, TrackStoreFlow.sub_season.state, [['C', 'None'], ['D', 'B', 'A'], ['Back']]),
        (TrackStoreStep.COVER_SOURCE, TrackStoreFlow.cover_source.state, [['Auto'], ['Album'], ['Back']]),
        (TrackStoreStep.ALBUM, TrackStoreFlow.album.state, [['Album A'], ['Back']]),
    ],
)
async def test_track_store_final_back_returns_to_origin_menu(
    from_step: TrackStoreStep,
    expected_state: str,
    expected_rows: list[list[str]],
) -> None:
    settings = _settings()
    album_id = '018f05c1f1a37b348d291f53a1ca4c1'
    year = date.today().year - 1
    track_store = SimpleNamespace(
        list_tracks=AsyncMock(
            return_value={
                track_store_module.SubSeason.A: [
                    track_store_module.TrackInfo(
                        id='018f05c1f1a37b348d291f53a1ca4c2',
                        album_id=album_id,
                        artists=('artist',),
                        title='Album A',
                        has_instrumental=False,
    ...
    buffer = ChatMessageBuffer()
    if from_step is TrackStoreStep.SUB_SEASON:
        buffer.append(
            _fake_message(chat_id=42, message_id=1, photo=_fake_photo(file_id='photo-1'), caption='Artist\nTitle'),
            chat_id=42,
        )
    ...
    elif from_step is TrackStoreStep.ALBUM:
        buffer.append(_fake_message(chat_id=42, message_id=1, text='Artist\nTitle'), chat_id=42)
        buffer.append(
            _fake_message(chat_id=42, message_id=2, audio=_fake_audio(file_id='audio-1', file_name='one.mp3')),
            chat_id=42,
    ...
    services = _services(clip_store=SimpleNamespace(), track_store=track_store, buffer=buffer)
    menu_message = _fake_message(text='Select action:', chat_id=42, message_id=9881)
    state = _FakeState()
    bot = SimpleNamespace(get_file=AsyncMock(), download_file=AsyncMock())
    ...
    for callback_data in base_steps:
        await on_track_store_menu(_fake_callback(menu_message), callback_data, state, services, settings, bot)
    ...
    await on_track_store_menu(
        _fake_callback(menu_message),
        TrackStoreCallbackData(action=TrackStoreAction.BACK, step=TrackStoreStep.FINAL, value='back'),
        state,
        services,
        settings,
        bot,
    ...
    assert _keyboard_rows(menu_message.edit_text.await_args.kwargs['reply_markup']) == expected_rows


@pytest.mark.asyncio
async def test_track_store_final_back_from_album_stale_when_album_options_disappear() -> None:
    settings = _settings()
    album_id = '018f05c1f1a37b348d291f53a1ca4e1'
    year = date.today().year - 1
    track_store = SimpleNamespace(
        list_tracks=AsyncMock(
            side_effect=[
                {
                    track_store_module.SubSeason.A: [
                        track_store_module.TrackInfo(
                            id='018f05c1f1a37b348d291f53a1ca4e2',
                            album_id=album_id,
                            artists=('artist',),
                            title='Album A',
                            has_instrumental=False,
    ...
    services = _services(clip_store=SimpleNamespace(), track_store=track_store, buffer=buffer)
    menu_message = _fake_message(text='Select action:', chat_id=42, message_id=9883)
    state = _FakeState()
    bot = SimpleNamespace(get_file=AsyncMock(), download_file=AsyncMock())
...
@pytest.mark.asyncio
async def test_track_store_fetch_mode_uses_stored_track_ids_and_skips_done(monkeypatch: pytest.MonkeyPatch) -> None:
    settings = _settings()
    stored_track_id = '018f05c1f1a37b348d291f53a1ca4d1'
    track_store = SimpleNamespace(
        store=AsyncMock(
            return_value=track_store_module.TrackInfo(
                id=stored_track_id,
                album_id='018f05c1f1a37b348d291f53a1ca4d2',
                artists=('artist',),
                title='title',
                has_instrumental=False,
                variant_mode='generated',
    ...
    send_fetched_tracks_by_ids = AsyncMock()
    monkeypatch.setattr(track_ingest_module, 'send_fetched_tracks_by_ids', send_fetched_tracks_by_ids)
    monkeypatch.setattr(track_store_execution_module, 'normalize_cover_to_jpg', Mock(return_value=b'jpg'))
    monkeypatch.setattr(track_store_execution_module, 'to_opus', AsyncMock(return_value=b'opus'))

    buffer = ChatMessageBuffer()
    buffer.append(
        _fake_message(chat_id=42, message_id=1, photo=_fake_photo(file_id='photo-1'), caption='A\nB'), chat_id=42
    )
    buffer.append(
        _fake_message(chat_id=42, message_id=2, audio=_fake_audio(file_id='audio-1', file_name='one.mp3')),
        chat_id=42,
    ...
    services = _services(clip_store=SimpleNamespace(), track_store=track_store, buffer=buffer)
    menu_message = _fake_message(text='Select action:', chat_id=42, message_id=9882)
    state = _FakeState()
    bot = SimpleNamespace(
        get_file=AsyncMock(
            side_effect=[SimpleNamespace(file_path='cover-path'), SimpleNamespace(file_path='audio-path')]
        ),
        download_file=AsyncMock(side_effect=[BytesIO(b'cover'), BytesIO(b'audio')]),
    )
    year = date.today().year - 1
    ...
    for callback_data in [
        ...
        TrackStoreCallbackData(action=TrackStoreAction.SELECT, step=TrackStoreStep.SEASON, value='1'),
        TrackStoreCallbackData(action=TrackStoreAction.SELECT, step=TrackStoreStep.SUB_SEASON, value='A'),
        TrackStoreCallbackData(action=TrackStoreAction.SELECT, step=TrackStoreStep.FINAL, value='fetch'),
    ]:
        await on_track_store_menu(_fake_callback(menu_message), callback_data, state, services, settings, bot)

    send_fetched_tracks_by_ids.assert_awaited_once()
    kwargs = send_fetched_tracks_by_ids.await_args.kwargs
    assert kwargs['track_ids'] == [stored_track_id]
    assert kwargs['chat_id'] == 42
    menu_message.answer.assert_not_awaited()
...
@pytest.mark.asyncio
async def test_route_action_compacts_external_route_before_success_summary() -> None:
    message = _fake_message(text='Select action:', chat_id=77, message_id=70)
    callback = _fake_callback(message)
    state = _FakeState()
    buffer = ChatMessageBuffer()
    ...
    async def record_summary(*args: object, **kwargs: object) -> None:
        del args, kwargs
        events.append('summary')

    async def record_compaction(*args: object, **kwargs: object) -> None:
        del kwargs
        clip_sub_group = args[1]
        assert clip_sub_group.scope in {Scope.SOURCE, Scope.EXTRA}
        assert message.answer.await_count == 0
        events.append(f'compact-{clip_sub_group.scope.name.lower()}')

    clip_store = SimpleNamespace(
        store=AsyncMock(return_value=StoreResult(stored_count=1, duplicate_count=0)),
        compact=AsyncMock(side_effect=record_compaction),
    )
    services = _services(clip_store=clip_store, buffer=buffer)

    message.answer.side_effect = record_summary
    bot = AsyncMock()
    ...
    assert events == ['compact-source', 'compact-extra', 'summary']
...
@pytest.mark.asyncio
async def test_route_action_does_not_compact_when_external_route_stores_nothing() -> None:
    message = _fake_message(text='Select action:', chat_id=77, message_id=70)
    callback = _fake_callback(message)
    state = _FakeState()
    buffer = ChatMessageBuffer()
...
@pytest.mark.asyncio
async def test_route_action_does_not_send_success_summary_until_external_compaction_finishes() -> None:
    message = _fake_message(text='Select action:', chat_id=77, message_id=70)
    callback = _fake_callback(message)
    state = _FakeState()
    buffer = ChatMessageBuffer()
    ...
    async def fail_on_extra(*args: object, **kwargs: object) -> None:
        del kwargs
        clip_sub_group = args[1]
        if clip_sub_group.scope is Scope.EXTRA:
            raise RuntimeError('boom')

    clip_store = SimpleNamespace(
        store=AsyncMock(return_value=StoreResult(stored_count=1, duplicate_count=0)),
        compact=AsyncMock(side_effect=fail_on_extra),
    )
    services = _services(clip_store=clip_store, buffer=buffer)
    ...
    message.answer.assert_not_awaited()
    assert clip_store.compact.await_count == 2
...
@pytest.mark.asyncio
async def test_route_action_uses_internal_route_destination_from_caption_only_stored_clip() -> None:
    message = _fake_message(text='Select action:', chat_id=77, message_id=79)
    callback = _fake_callback(message)
    state = _FakeState()
    buffer = ChatMessageBuffer()
    destination_group = ClipGroup(universe=Universe.EAST, year=2024, season=Season.S2)
    source_group = ClipGroup(universe=Universe.WEST, year=2024, season=Season.S1)
    source_sub_group = ClipSubGroup(sub_season=SubSeason.NONE, scope=Scope.EXTRA)
    buffer.append(
        _fake_message(
    ...
    _assert_format_kwargs(
        ...
        {**_transfer_progress_kwargs(('East', '2024', '2')), 'reply_markup': None},
    )
...
@pytest.mark.asyncio
async def test_route_action_internal_caption_destination_carries_forward_to_uncaptioned_stored_clips() -> None:
    message = _fake_message(text='Select action:', chat_id=77, message_id=79)
    callback = _fake_callback(message)
    state = _FakeState()
    buffer = ChatMessageBuffer()
    ...
    assert clip_store.transfer.await_args.kwargs['clips'] == [
        intake_module.TransferClipRef(source_group=source_group, clip_id=_CLIP_ID_1),
        intake_module.TransferClipRef(source_group=source_group, clip_id=_CLIP_ID_2),
    ]
    assert message.edit_text.await_args_list[0] == call('Routing...', reply_markup=None)
    _assert_format_kwargs(
        message.edit_text.await_args_list[1].kwargs,
    ...
    message.answer.assert_awaited_once_with(**Text('Transferred: ', Bold('2')).as_kwargs())
    assert clip_store.compact.await_args_list == [
        call(
            source_group,
            source_sub_group,
...
@pytest.mark.asyncio
async def test_route_action_internal_route_updates_destination_across_multiple_caption_changes() -> None:
    message = _fake_message(text='Select action:', chat_id=77, message_id=82)
    callback = _fake_callback(message)
    state = _FakeState()
    buffer = ChatMessageBuffer()
    west_destination = ClipGroup(universe=Universe.WEST, year=2025, season=Season.S1)
    east_destination = ClipGroup(universe=Universe.EAST, year=2024, season=Season.S2)
    source_group = ClipGroup(universe=Universe.WEST, year=2024, season=Season.S1)
    source_sub_group = ClipSubGroup(sub_season=SubSeason.NONE, scope=Scope.EXTRA)
    for buffered_message in [
        _fake_message(
        ...
        _fake_message(
            chat_id=77,
            message_id=3,
            video=_fake_video(file_id='f3', file_name=_stored_filename(source_group, source_sub_group, _CLIP_ID_3)),
        ),
    ]:
        buffer.append(buffered_message, chat_id=77)
    clip_store = SimpleNamespace(
        transfer=AsyncMock(
            side_effect=[
                TransferResult(transferred_count=1, already_in_destination_group_count=0, duplicate_blocked_count=0),
                TransferResult(transferred_count=2, already_in_destination_group_count=0, duplicate_blocked_count=0),
            ]
        ),
        compact=AsyncMock(),
    )
    ...
    assert clip_store.transfer.await_count == 2
    assert clip_store.transfer.await_args_list[0].kwargs['destination_group'] == west_destination
    assert clip_store.transfer.await_args_list[0].kwargs['clips'] == [
        intake_module.TransferClipRef(source_group=source_group, clip_id=_CLIP_ID_1)
    ]
    assert clip_store.transfer.await_args_list[1].kwargs['destination_group'] == east_destination
    assert clip_store.transfer.await_args_list[1].kwargs['clips'] == [
        intake_module.TransferClipRef(source_group=source_group, clip_id=_CLIP_ID_2),
        intake_module.TransferClipRef(source_group=source_group, clip_id=_CLIP_ID_3),
    ]


@pytest.mark.asyncio
async def test_route_action_accepts_internal_mp4_documents() -> None:
    message = _fake_message(text='Select action:', chat_id=77, message_id=82)
    callback = _fake_callback(message)
    state = _FakeState()
    buffer = ChatMessageBuffer()
...
@pytest.mark.asyncio
async def test_route_action_compacts_internal_route_before_success_summary() -> None:
    message = _fake_message(text='Select action:', chat_id=77, message_id=79)
    callback = _fake_callback(message)
    state = _FakeState()
    buffer = ChatMessageBuffer()
    ...
    async def record_compaction(*args: object, **kwargs: object) -> None:
        ...
        clip_group = args[0]
        assert message.answer.await_count == 0
        if clip_group == source_group:
            events.append('compact-source')
        else:
            assert clip_group == destination_group
            events.append('compact-destination')
    ...
    assert events == ['compact-source', 'compact-destination', 'summary']
    message.answer.assert_awaited_once_with(**Text('Transferred: ', Bold('1')).as_kwargs())
    assert clip_store.compact.await_args_list == [
        call(
            source_group,
...
@pytest.mark.asyncio
async def test_route_action_does_not_send_success_summary_until_internal_compaction_finishes() -> None:
    message = _fake_message(text='Select action:', chat_id=77, message_id=79)
    callback = _fake_callback(message)
    state = _FakeState()
    buffer = ChatMessageBuffer()
    ...
    async def fail_on_destination(*args: object, **kwargs: object) -> None:
        del kwargs
        clip_group = args[0]
        if clip_group == source_group:
            return
    ...
    clip_store = SimpleNamespace(
        ...
        compact=AsyncMock(side_effect=fail_on_destination),
    )
    services = _services(clip_store=clip_store, buffer=buffer)
...
@pytest.mark.asyncio
async def test_route_action_internal_route_standalone_text_then_caption_updates_destination() -> None:
    message = _fake_message(text='Select action:', chat_id=77, message_id=80)
    callback = _fake_callback(message)
    state = _FakeState()
    buffer = ChatMessageBuffer()
    ...
    for buffered_message in [
        _fake_message(chat_id=77, message_id=1, text='w251'),
        _fake_message(
            chat_id=77,
            message_id=2,
            video=_fake_video(file_id='f1', file_name=_stored_filename(source_group, source_sub_group, _CLIP_ID_1)),
...
@pytest.mark.asyncio
async def test_route_action_internal_route_caption_then_standalone_text_updates_destination() -> None:
    message = _fake_message(text='Select action:', chat_id=77, message_id=81)
    callback = _fake_callback(message)
    state = _FakeState()
    buffer = ChatMessageBuffer()
    ...
    for buffered_message in [
        ...
        _fake_message(chat_id=77, message_id=2, text='e242'),
        _fake_message(
            chat_id=77,
            message_id=3,
            video=_fake_video(file_id='f2', file_name=_stored_filename(source_group, source_sub_group, _CLIP_ID_2)),
...
@pytest.mark.asyncio
async def test_route_action_reports_internal_route_counts() -> None:
    message = _fake_message(text='Select action:', chat_id=77, message_id=80)
    callback = _fake_callback(message)
    state = _FakeState()
    buffer = ChatMessageBuffer()
...
@pytest.mark.asyncio
async def test_legacy_transfer_action_rejects_fresh_clip_identity_input() -> None:
    message = _fake_message(text='Select action:', chat_id=77, message_id=81)
    callback = _fake_callback(message)
    state = _FakeState()
    buffer = ChatMessageBuffer()
...
async def test_route_action_surfaces_stale_internal_references_as_external_clips(
...
@pytest.mark.asyncio
async def test_route_action_surfaces_stale_same_group_internal_reference_as_external_clip() -> None:
    message = _fake_message(text='Select action:', chat_id=77, message_id=84)
    callback = _fake_callback(message)
    state = _FakeState()
    buffer = ChatMessageBuffer()
...
@pytest.mark.asyncio
async def test_route_action_rejects_mixed_external_and_internal_clips_before_side_effects() -> None:
    message = _fake_message(text='Select action:', chat_id=77, message_id=85)
    callback = _fake_callback(message)
    state = _FakeState()
    buffer = ChatMessageBuffer()
    ...
    buffer.append(
        _fake_message(
            chat_id=77,
            message_id=3,
            video=_fake_video(file_id='f2', file_name=_stored_filename(source_group, source_sub_group, _CLIP_ID_1)),
        ),
        chat_id=77,
    )
    clip_store = SimpleNamespace(store=AsyncMock(), transfer=AsyncMock(), compact=AsyncMock())
    bot = AsyncMock()
    services = _services(clip_store=clip_store, buffer=buffer)
...
@pytest.mark.asyncio
async def test_route_action_requires_physical_group_for_internal_route() -> None:
    message = _fake_message(text='Select action:', chat_id=77, message_id=86)
    callback = _fake_callback(message)
    state = _FakeState()
    buffer = ChatMessageBuffer()
    source_group = ClipGroup(universe=Universe.WEST, year=2024, season=Season.S1)
    source_sub_group = ClipSubGroup(sub_season=SubSeason.NONE, scope=Scope.EXTRA)
    buffer.append(_fake_message(chat_id=77, message_id=1, text='w251a'), chat_id=77)
    buffer.append(
        _fake_message(
            chat_id=77,
            message_id=2,
    ...
    message.edit_text.assert_awaited_once_with('Physical group required', reply_markup=None)
    clip_store.transfer.assert_not_awaited()
    clip_store.compact.assert_not_awaited()


@pytest.mark.asyncio
async def test_route_action_requires_physical_group_for_internal_route_caption() -> None:
    message = _fake_message(text='Select action:', chat_id=77, message_id=87)
    callback = _fake_callback(message)
    state = _FakeState()
    buffer = ChatMessageBuffer()
    source_group = ClipGroup(universe=Universe.WEST, year=2024, season=Season.S1)
